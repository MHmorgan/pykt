...
class IniFile(
    ...
    val sectionNames: Set<String> get() = _originalSectionNames.values.toSet()
    
    /** Get all sections. */
    val sections: Map<String, IniSection> get() = _sections.values.associateBy { _originalSectionNames[getSectionKey(it.name)] ?: it.name }
    ...
    fun toFlatMap(): Map<String, String> {
        ...
        for ((sectionKey, section) in _sections) {
            ...
            for ((key, value) in section.entries) {
                val flatKey = if (originalSectionName == dialect.defaultSectionName) key else "$originalSectionName.$key"
